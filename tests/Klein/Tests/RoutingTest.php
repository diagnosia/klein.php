...
class RoutingTest extends AbstractKleinTestCase
{
    public function testBasic()
    {
        ...
        $this->klein_app->respond('/something', function () {
            echo 'y';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/'));
    }
    ...
    public function testCallbackArguments()
    {
        ...
        $this->klein_app->respond(function ($a, $b, $c, $d, $e, $f, $g) use (&$expected_objects) {
    ...
    public function testAppReference()
    {
        ...
        $this->klein_app->respond('/', function ($request, $response, $service, $app) {
            $app->state = 'a';
        });
        $this->klein_app->respond('/', function ($request, $response, $service, $app) {
            $app->state .= 'b';
    ...
    public function testDispatchOutput()
    {
        ...
        $this->klein_app->respond(function () use ($expected_output) {
        ...
        $this->expectOutputString($expected_output['returned1'] . $expected_output['returned2']);
    ...
    public function testDispatchOutputNotSent()
    {
        ...
        $this->assertSame('test output', $this->klein_app->response()->body());
    }
    ...
    public function testDispatchResponseReplaced()
    {
        ...
        $this->klein_app->respond('/', function () use ($expected_body, $expected_code) {
            return new Response($expected_body, $expected_code);
        });
        $this->klein_app->respond('/', function () use ($expected_append) {
        ...
        $this->assertSame($expected_body . $expected_append, $this->klein_app->response()->body());
        $this->assertSame($expected_code, $this->klein_app->response()->code());
    }

    public function testRespondReturn()
    {
        $return_one = $this->klein_app->respond(function () {
            return 1337;
        });
        $return_two = $this->klein_app->respond(function () {
            return 'dog';
        });
    ...
    public function testCatchallImplicit()
    {
        ...
        $this->klein_app->respond('/one', function () {
            echo 'a';
        });
        $this->klein_app->respond(function () {
            echo 'b';
        ...
        $this->klein_app->respond('/three', function () {
            echo 'c';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/two'));
    }
    ...
    public function testCatchallImplicitTriggers404()
    {
        ...
        $this->klein_app->onHttpError(function ($code) {
            if (404 === $code) {
                echo "404\n";
            }
        });
    ...
    public function testRegex()
    {
        ...
        $this->klein_app->respond('@/bar', function () {
            echo 'z';
        });

        $this->klein_app->respond('@/[0-9]s', function () {
            echo 'z';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/bar'));
        $this->klein_app->dispatch(MockRequestFactory::create('/8s'));
        $this->klein_app->dispatch(MockRequestFactory::create('/88s'));
    }
    ...
    public function testRegexNegate()
    {
        ...
        $this->klein_app->respond('!@/foo', function () {
            echo 'y';
        });
    ...
    public function testNormalNegate()
    {
        ...
        $this->klein_app->dispatch(MockRequestFactory::create('/foo'));
    }
    ...
    public function testParamsBasic()
    {
        ...
        $this->klein_app->respond('/[:color]', function ($request) {
            echo $request->param('color');
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/blue'));
    }
    ...
    public function testParamsIntegerSuccess()
    {
        ...
        $this->klein_app->respond('/[i:age]', function ($request) {
            $age = $request->param('age');

            printf('%s(%d) "%s"', gettype($age), strlen($age), $age);
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/987'));
    }
    ...
    {
        $this->klein_app->respond('/[a:audible]', function ($request) {
            echo $request->param('audible');
        });

        $this->assertSame('blue42', $this->dispatchAndReturnOutput(MockRequestFactory::create('/blue42')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/texas-29')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/texas29!')));
    }

    public function testParamsHex()
    {
        $this->klein_app->respond('/[h:hexcolor]', function ($request) {
            echo $request->param('hexcolor');
        });

        $this->assertSame('00f', $this->dispatchAndReturnOutput(MockRequestFactory::create('/00f')));
        $this->assertSame('abc123', $this->dispatchAndReturnOutput(MockRequestFactory::create('/abc123')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/876zih')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/00g')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/hi23')));
    }

    public function testParamsSlug()
    {
        $this->klein_app->respond('/[s:slug_name]', function ($request) {
            echo $request->param('slug_name');
        });

        $this->assertSame('dog-thing', $this->dispatchAndReturnOutput(MockRequestFactory::create('/dog-thing')));
        $this->assertSame(
            'a_badass_slug',
            $this->dispatchAndReturnOutput(MockRequestFactory::create('/a_badass_slug')),
        );
        $this->assertSame(
            'AN_UPERCASE_SLUG',
            $this->dispatchAndReturnOutput(MockRequestFactory::create('/AN_UPERCASE_SLUG')),
        );
        $this->assertSame(
            'sample-wordpress-like-post-slug-based-on-the-title-2013-edition',
            $this->dispatchAndReturnOutput(MockRequestFactory::create(
                '/sample-wordpress-like-post-slug-based-on-the-title-2013-edition',
            )),
        );
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/%!@#')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/')));
        $this->assertSame('', $this->dispatchAndReturnOutput(MockRequestFactory::create('/dog-%thing')));
    }

    public function testPathParamsAreUrlDecoded()
    {
        $this->klein_app->respond('/[:test]', function ($request) {
            echo $request->param('test');
        });

        $this->assertSame('Knife Party', $this->dispatchAndReturnOutput(MockRequestFactory::create('/Knife%20Party')));

        $this->assertSame('and/or', $this->dispatchAndReturnOutput(MockRequestFactory::create('/and%2For')));
    }
    ...
    public function testPathParamsAreUrlDecodedToRFC3986Spec()
    {
        ...
        $this->assertNotSame('Knife Party', $this->dispatchAndReturnOutput(MockRequestFactory::create('/Knife+Party')));

        $this->assertSame('Knife+Party', $this->dispatchAndReturnOutput(MockRequestFactory::create('/Knife+Party')));
    }
    ...
    public function test404RouteDefinitionOrderDoesntEffectWhen404HandlersCalled()
    {
        ...
        $this->klein_app->respond('404', function () {
            echo '404 Code';
        });
        $this->klein_app->respond(function () {
            echo 'two';
    ...
    public function testMethodCatchAll()
    {
        ...
        $this->klein_app->respond('POST', null, function ($request) {
            echo 'yup!';
        });
        $this->klein_app->respond('POST', '*', function ($request) {
            echo '1';
        });
        $this->klein_app->respond('POST', '/', function ($request) {
            echo '2';
        });
        $this->klein_app->respond(function ($request) {
            echo '3';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/', 'POST'));
    }
    ...
    public function testLazyTrailingMatch()
    {
        ...
        $this->klein_app->respond('/posts/[*:title][i:id]', function ($request) {
            echo $request->param('title') . $request->param('id');
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/posts/this-is-a-title-123'));
    }
    ...
    public function testFormatMatch()
    {
        ...
        $this->klein_app->dispatch(MockRequestFactory::create('/output.xml'));
    }
    ...
    public function testDotSeparator()
    {
        ...
        $this->klein_app->respond('/[*:cpath]/[:slug].[:format]', function ($rq) {
            echo 'matchA:slug=' . $rq->param('slug') . '--';
        });
        $this->klein_app->respond('/[*:cpath]/[:slug].[:format]?', function ($rq) {
            echo 'matchB:slug=' . $rq->param('slug') . '--';
        });
        $this->klein_app->respond('/[*:cpath]/[a:slug].[:format]?', function ($rq) {
            echo 'matchC:slug=' . $rq->param('slug') . '--';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/category1/categoryX/ABCD_E.php'));

        $this->assertSame(
            'matchA:slug=ABCD_E--matchB:slug=ABCD_E--',
            $this->dispatchAndReturnOutput(MockRequestFactory::create('/category1/categoryX/ABCD_E.php')),
        );
        $this->assertSame(
            'matchB:slug=ABCD_E--',
            $this->dispatchAndReturnOutput(MockRequestFactory::create('/category1/categoryX/ABCD_E')),
        );
    }
    ...
    public function testControllerActionStyleRouteMatch()
    {
        ...
        $this->klein_app->respond('/[:controller]?/[:action]?', function ($request) {
            echo $request->param('controller') . '-' . $request->param('action');
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/donkey/kick'));
    }
    ...
    public function testRespondArgumentOrder()
    {
        ...
        $this->klein_app->respond(null, function () {
            echo 'b';
        });
        $this->klein_app->respond('/endpoint', function () {
            echo 'c';
        });
        $this->klein_app->respond('GET', null, function () {
            echo 'd';
        });
        $this->klein_app->respond(array('GET', 'POST'), null, function () {
            echo 'e';
        });
        $this->klein_app->respond(array('GET', 'POST'), '/endpoint', function () {
            echo 'f';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/endpoint'));
    }

    public function testTrailingMatch()
    {
        $this->klein_app->respond('/?[*:trailing]/dog/?', function ($request) {
            echo 'yup';
        });

        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/cat/dog')));
        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/cat/cheese/dog')));
        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/cat/ball/cheese/dog/')));
        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/cat/ball/cheese/dog')));
        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('cat/ball/cheese/dog/')));
        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('cat/ball/cheese/dog')));
    }
    ...
    {
        $this->klein_app->respond('/sub-dir/[**:trailing]', function ($request) {
            echo 'yup';
        });

        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/sub-dir/dog')));

        $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create('/sub-dir/cheese/dog')));

        $this->assertSame(
            'yup',
            $this->dispatchAndReturnOutput(MockRequestFactory::create('/sub-dir/ball/cheese/dog/')),
        );
    ...
    {
        $this->klein_app->with('/u', function ($klein_app) {
            $klein_app->respond('GET', '/?', function ($request, $response) {
                echo 'slash';
            });
            $klein_app->respond('GET', '/[:id]', function ($request, $response) {
                echo 'id';
            });
        });
        ...
        $this->assertSame('slash', $this->dispatchAndReturnOutput(MockRequestFactory::create('/u')));
        $this->assertSame('slash', $this->dispatchAndReturnOutput(MockRequestFactory::create('/u/')));
        $this->assertSame('id', $this->dispatchAndReturnOutput(MockRequestFactory::create('/u/35')));
        $this->assertSame('404', $this->dispatchAndReturnOutput(MockRequestFactory::create('/35')));
    }
    ...
    public function testNSDispatchExternal()
    {
        ...
        $this->klein_app->respond(404, function ($request, $response) {
            echo '404';
        });

        foreach ($ext_namespaces as $namespace) {
            $this->assertSame('yup', $this->dispatchAndReturnOutput(MockRequestFactory::create($namespace . '/')));

            $this->assertSame(
                'yup',
                $this->dispatchAndReturnOutput(MockRequestFactory::create($namespace . '/testing/')),
            );
        }
    }
    ...
    {
        $this->klein_app->respond(array('GET', 'POST'), '/', function () {
            echo 'fail';
        });
    ...
    public function test405Routes()
    {
        ...
        $this->klein_app->respond('GET', '/sure', function () {
            echo 'fail';
        });
        $this->klein_app->respond(array('GET', 'POST'), '/sure', function () {
            echo 'fail';
        });
        $this->klein_app->respond(405, function ($a, $b, $c, $d, $e, $f, $methods) use (&$result_array) {
        ...
        $this->klein_app->dispatch(MockRequestFactory::create('/sure', 'DELETE'));
    ...
    public function test405ErrorHandler()
    {
        ...
        $this->klein_app->onHttpError(function ($code, $klein, $matched, $methods, $exception) use (&$result_array) {
            $result_array = $methods;
        });
    ...
    public function testOptionsDefaultRequest()
    {
        ...
        $this->klein_app->dispatch(MockRequestFactory::create('/', 'OPTIONS'));
    ...
    public function testOptionsRoutes()
    {
        ...
        $this->klein_app->respond('GET', '/', function () {
            echo 'fail';
        });
        $this->klein_app->respond(array('GET', 'POST'), '/', function () {
            echo 'fail';
        });
        $this->klein_app->respond('OPTIONS', null, function ($request, $response) use ($access_control_headers) {
    ...
    public function testHeadDefaultRequest()
    {
        ...
        $this->klein_app->respond('GET', null, function ($request, $response) use ($expected_headers) {
            $response->code(200);
        ...
        $this->klein_app->respond('POST', '/', function () {
            echo 'POST!';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/', 'HEAD'));
    ...
    public function testHeadMethodMatch()
    {
        ...
        $this->klein_app->respond(array('GET', 'HEAD'), null, function ($request, $response) use (
            $test_strings,
            &$test_result,
        ) {
            $test_result .= $test_strings[0];
        });
        $this->klein_app->respond('GET', '/', function ($request, $response) use ($test_strings, &$test_result) {
            $test_result .= $test_strings[1];
        });
        $this->klein_app->respond('POST', '/', function ($request, $response) use ($test_strings, &$test_result) {
            $test_result .= 'nope';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/', 'HEAD'));

        $this->assertSame(implode('', $test_strings), $test_result);
    }

    public function testGetPathFor()
    {
        $this->klein_app
            ->respond('/dogs', function () {})
            ->setName('dogs');

        $this->klein_app
            ->respond('/dogs/[i:dog_id]/collars', function () {})
            ->setName('dog-collars');

        $this->klein_app
            ->respond('/dogs/[i:dog_id]/collars/[a:collar_slug]/?', function () {})
            ->setName('dog-collar-details');

        $this->klein_app
            ->respond('/dog/foo', function () {})
            ->setName('dog-foo');

        $this->klein_app
            ->respond('/dog/[i:dog_id]?', function () {})
            ->setName('dog-optional-details');

        $this->klein_app
            ->respond('@/dog/regex', function () {})
            ->setName('dog-regex');
        ...
        $this->assertSame('/dogs', $this->klein_app->getPathFor('dogs'));
        $this->assertSame('/dogs/[i:dog_id]/collars', $this->klein_app->getPathFor('dog-collars'));
        $this->assertSame('/dogs/idnumberandstuff/collars', $this->klein_app->getPathFor('dog-collars', array(
        ...
        $this->assertSame('/dogs/idnumberandstuff/collars/d12f3d1f2d3/?', $this->klein_app->getPathFor('dog-collar-details', array(
            'dog_id' => 'idnumberandstuff',
            'collar_slug' => 'd12f3d1f2d3',
        )));
        $this->assertSame('/dog/foo', $this->klein_app->getPathFor('dog-foo'));
        $this->assertSame('/dog', $this->klein_app->getPathFor('dog-optional-details'));
        $this->assertSame('/', $this->klein_app->getPathFor('dog-regex'));
        $this->assertSame('/', $this->klein_app->getPathFor('dog-neg-regex'));
        $this->assertSame('@/dog/regex', $this->klein_app->getPathFor('dog-regex', null, false));
        $this->assertNotSame('/', $this->klein_app->getPathFor('dog-neg-regex', null, false));
        $this->assertSame('/', $this->klein_app->getPathFor('complex-regex'));
        $this->assertSame('/', $this->klein_app->getPathFor('complex-neg-regex'));
        $this->assertSame('@\.(json|csv)$', $this->klein_app->getPathFor('complex-regex', null, false));
        $this->assertNotSame('/', $this->klein_app->getPathFor('complex-neg-regex', null, false));
    }
    ...
    public function testDispatchHalt()
    {
        ...
        $this->klein_app->respond(function ($a, $b, $c, $d, $klein_app) {
            $klein_app->skipThis();
            echo '1,';
        });
        $this->klein_app->respond(function ($a, $b, $c, $d, $klein_app) {
    ...
    public function testDispatchSkipCauses404()
    {
        ...
        $this->klein_app->respond('POST', '/steez', function ($a, $b, $c, $d, $klein_app) {
            $klein_app->skipThis();
            echo 'Style... with ease';
        });
        $this->klein_app->respond('GET', '/nope', function ($a, $b, $c, $d, $klein_app) {
            echo 'How did I get here?!';
        });

        $this->klein_app->dispatch(MockRequestFactory::create('/steez', 'POST'));
    }
    ...
    public function testDispatchAbortCallsHttpError()
    {
        ...
        $this->klein_app->onHttpError(function ($code, $klein_app) {
            echo 'aborted,';
            echo $code;
        });
    ...
    public function testDispatchExceptionRethrowsUnknownCode()
    {
        ...
        $this->klein_app->respond(function ($a, $b, $c, $d, $klein_app) use ($test_message, $test_code) {
    ...
    public function testOptionsAlias()
    {
        ...
        // With path
        $this->klein_app->options('/', function () {
            echo '1,';
        });
    ...
    public function testHeadAlias()
    {
        ...
        // With path
        $this->klein_app->head('/', function ($request, $response) {
            echo '1,';
    ...
    public function testPutAlias()
    {
        ...
        $this->klein_app->dispatch(MockRequestFactory::create('/', 'PUT'));
    }
    ...
    {
        $this->klein_app->respond('/[:a]', function ($request) {
    ...
    {
        $this->klein_app->respond('/[:foo]/[:bar]', function ($request) {
    ...
    public function testMatchesDotOutsideOfNamedParam()
    {
        ...
        $this->klein_app->respond('/[:file].[:ext]', function ($request) use (&$file, &$ext) {
    ...
    {
        $this->klein_app->respond('/file.ext', function () {});

        // Should match
        $this->klein_app->dispatch(MockRequestFactory::create('/file.ext'));
        $this->assertSame(200, $this->klein_app->response()->code());

        // Shouldn't match
        $this->klein_app->dispatch(MockRequestFactory::create('/file0ext'));
        $this->assertSame(404, $this->klein_app->response()->code());
    }
    ...
    {
        $this->klein_app->respond('/[a:site].[:format]?/[:id].[:format2]?', function () {});

        $this->klein_app->dispatch(MockRequestFactory::create('/site.main/id.json'));
        $this->assertSame(200, $this->klein_app->response()->code());
    }

    public function testMatchesLiteralPlusSignsInPaths()
    {
        $this->klein_app->respond('/te+st', function () {});

        // Should match
        $this->klein_app->dispatch(MockRequestFactory::create('/te+st'));
        $this->assertSame(200, $this->klein_app->response()->code());

        // Shouldn't match
        $this->klein_app->dispatch(MockRequestFactory::create('/teeeeeeeeest'));
        $this->assertSame(404, $this->klein_app->response()->code());
    }

    public function testMatchesParenthesesInPaths()
    {
        $this->klein_app->respond('/test(bar)', function () {});

        $this->klein_app->dispatch(MockRequestFactory::create('/test(bar)'));
        $this->assertSame(200, $this->klein_app->response()->code());
    }

    public function testMatchesAdvancedRegularExpressions()
    {
        $this->klein_app->respond('@^/foo.../bar$', function () {});

        $this->klein_app->dispatch(MockRequestFactory::create('/foooom/bar'));
        $this->assertSame(200, $this->klein_app->response()->code());
    }
